...
def analyze_depolarizing_data(
    code_distance: int, num_round: int, initial_state: int, num_shot: int
) -> np.ndarray:
    # load record
    filename = f"./record_sim/record_depolarizing_d{code_distance}_r{num_round}_i{initial_state}.json"
...
for code_distance in code_distance_list:
    for num_round in num_round_list:
        for initial_state in initial_state_list:
            ...
            filename = f"./correlation/correlation_matrix_depolarizing_d{code_distance}_r{num_round}_i{initial_state}.npy"
            matrix = analyze_depolarizing_data(
