...
def plot_correlation(
    ...
    dataset_filename = f"./correlation/correlation_matrix_simulate_d{code_distance}_r{num_round}_i{initial_state}.npy"
    with open(dataset_filename, "rb") as fin:
        matrix_sim = np.load(fin)

    dataset_filename = f"./correlation/correlation_matrix_uniform_d{code_distance}_r{num_round}_i{initial_state}.npy"
    with open(dataset_filename, "rb") as fin:
        matrix_uni = np.load(fin)
    ...
    plt.imshow(matrix_sim, cmap="seismic", vmin=-limit, vmax=limit)
    _format(num_detector, num_round, code_distance)

    plt.subplot(1, 3, 3)
    plt.title("Uniform")
    plt.imshow(matrix_uni, cmap="seismic", vmin=-limit, vmax=limit)
    _format(num_detector, num_round, code_distance)
    ...
    for code_distance in code_distance_list:
        for num_round in num_round_list:
            for initial_state in initial_state_list:
                ...
                filename = f"./figure/correlation_matrix_experiment_d{code_distance}_r{num_round}_i{initial_state}"
