...
def plot_correlation(
    ...
    dataset_filename = f"./correlation/correlation_matrix_coherent_XX_d{code_distance}_r{num_round}_i{initial_state}.npy"
    with open(dataset_filename, "rb") as fin:
        matrix_coherent = np.load(fin)
    ...
    plt.subplot(1, 3, 3)
    plt.title("Coherent")
    plt.imshow(matrix_coherent, cmap="seismic", vmin=-limit, vmax=limit)
    _format(num_detector, num_round, code_distance)
    ...
    for code_distance in code_distance_list:
        for num_round in num_round_list:
            for initial_state in initial_state_list:
                ...
                filename = f"./figure/correlation_matrix_coherent_XX_d{code_distance}_r{num_round}_i{initial_state}"
